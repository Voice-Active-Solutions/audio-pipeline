...
class CustomASRCallback(RecognizeCallback):
    ...
    def on_data(self, data):
        ...
        logger.info("ASR batch job completed:")
...
def load_audio_from_cos(cos, bucket_name, object_key, local_filename):
    ...
    try:
        ...
        logger.info(f"Audio file saved as '{local_filename}'")

    except ClientError as e:
        logger.error(f"Unable to stream file: {e}")


def main() -> None:
    logger.info(f"Starting AudioPipeline application version {VERSION}")
    ...
    logger.info(f"File {object_key} being read from from bucket: {bucket}")
